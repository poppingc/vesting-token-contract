...
abstract contract TokenVesting is ERC20, Ownable {
    ...
    /**
     * @dev 获取 下一解锁阶段可解锁的 单次金额
     * @param _beneficiary 受益人地址
     * @return uint256 token.balance
     */
    function nextReleaseAmount(address _beneficiary)
        public
        view
        returns (uint256)
    {
        if (block.timestamp >= endReleaseTime(_beneficiary)) {
            return 0;
        } else if (
            unReleaseAmount(_beneficiary) ==
            _vestingOf[_beneficiary].totalLockAmount &&
            _vestingOf[_beneficiary].firstRatio > 0
        ) {
            return _firstAmount(_beneficiary);
        } else if (
            nextReleaseTime(_beneficiary) >= endReleaseTime(_beneficiary)
        ) {
            return
                _vestingOf[_beneficiary].totalLockAmount -
                nowReleaseAllAmount(_beneficiary) +
                _vestingOf[_beneficiary].released;
        } else {
            if (_vestingOf[_beneficiary].customizeRatio.length > 0) {
                // 下一时间的定制解锁比率金额
                return
                    _vestedCustomizeRatioAmount(
                        _beneficiary,
                        _nextReleaseCount(_beneficiary)
                    );
            } else {
                // 获取 单次解锁金额
                return _singleReleaseAmount(_beneficiary);
            }
        }
    }
